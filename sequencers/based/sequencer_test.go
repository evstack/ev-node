...
func createTestSequencer(t *testing.T, mockDA *MockDA, cfg config.Config, gen genesis.Genesis) *BasedSequencer {
	...
	seq, err := NewBasedSequencer(context.Background(), fiRetriever, mockDA, db, cfg, gen, zerolog.Nop())
...
func TestBasedSequencer_CheckpointPersistence(t *testing.T) {
	...
	seq1, err := NewBasedSequencer(context.Background(), fiRetriever, mockDA, db, cfg, gen, zerolog.Nop())
	require.NoError(t, err)
	...
	seq2, err := NewBasedSequencer(context.Background(), fiRetriever, mockDA, db, cfg, gen, zerolog.Nop())
	require.NoError(t, err)
