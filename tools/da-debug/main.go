...
Connects directly to Celestia's blob API.`,
	}

	// Global flags
	rootCmd.PersistentFlags().StringVar(&daURL, "da-url", "http://localhost:26658", "Celestia node RPC URL")
	rootCmd.PersistentFlags().StringVar(&authToken, "auth-token", "", "Authentication token for Celestia node")
...
func searchForHeight(ctx context.Context, client *celestia.Client, startHeight uint64, namespace []byte, targetHeight, searchRange uint64) error {
	...
	for daHeight := startHeight; daHeight < startHeight+searchRange; daHeight++ {
		result, err := client.GetIDs(ctx, daHeight, namespace)
		if err != nil {
		...
		blobs, err := client.Get(ctx, result.IDs, namespace)
		if err != nil {
			continue
		}
...
func queryHeight(ctx context.Context, client *celestia.Client, height uint64, namespace []byte) error {
	result, err := client.GetIDs(ctx, height, namespace)
	if err != nil {
...
func createDAClient() (*celestia.Client, error) {
	...
	client, err := celestia.NewClient(ctx, logger, daURL, authToken, maxBlobSize)
	if err != nil {
...
func parseNamespace(ns string) ([]byte, error) {
	...
	if hexBytes, err := parseHex(ns); err == nil && len(hexBytes) == da.NamespaceSize {
		return hexBytes, nil
	}

	// If not valid hex or not 29 bytes, treat as string identifier
	namespace := da.NamespaceFromString(ns)
