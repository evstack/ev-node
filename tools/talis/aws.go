...
func ensureAWSKeyPair(ctx context.Context, client *ec2.Client, keyName, publicKey string) error {
	...
	if _, err := client.ImportKeyPair(ctx, &ec2.ImportKeyPairInput{
		...
		// `talis up` parallelises CreateAWSInstances, so each
		// goroutine races to import the same key. Only one wins;
		// the rest see Duplicate. Treat that as success — the key
