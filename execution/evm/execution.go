...
func (c *EngineClient) ExecuteTxs(ctx context.Context, txs [][]byte, blockHeight uint64, timestamp time.Time, prevStateRoot []byte) (updatedStateRoot []byte, maxBytes uint64, err error) {
	// convert evolve node tx to hex strings for ev-reth
	...
	// Create ev-node-compatible payload attributes with flattened structure
	evnodePayloadAttrs := map[string]interface{}{
		...
		"parentBeaconBlockRoot": common.Hash{}.Hex(), // Use zero hash for evnode
		// evnode-specific fields
