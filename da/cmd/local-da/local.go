...
// kvp is a key-value pair used by the datastore implementation.
type kvp struct {
	key, value []byte
}

// NewLocalDA create new instance of DummyDA
func NewLocalDA(logger *zap.Logger, opts ...func(*LocalDA) *LocalDA) *LocalDA {
	da := &LocalDA{
		mu:          new(sync.Mutex),
...
func (d *LocalDA) MaxBlobSize(ctx context.Context) (uint64, error) {
	d.logger.Debug("MaxBlobSize called", zap.Uint64("maxBlobSize", d.maxBlobSize))
...
func (d *LocalDA) Get(ctx context.Context, ids []coreda.ID, _ []byte) ([]coreda.Blob, error) {
	d.logger.Debug("Get called", zap.Any("ids", ids))
	d.mu.Lock()
	defer d.mu.Unlock()
	blobs := make([]coreda.Blob, len(ids))
	for i, id := range ids {
		if len(id) < 8 {
			d.logger.Error("Get: invalid ID length", zap.ByteString("id", id))
		...
		if !found {
			d.logger.Warn("Get: blob not found", zap.ByteString("id", id), zap.Uint64("height", height))
			return nil, coreda.ErrBlobNotFound
		}
	}
	d.logger.Debug("Get successful", zap.Int("count", len(blobs)))
	return blobs, nil
}
...
func (d *LocalDA) GetIDs(ctx context.Context, height uint64, _ []byte) (*coreda.GetIDsResult, error) {
	d.logger.Debug("GetIDs called", zap.Uint64("height", height))
	d.mu.Lock()
	defer d.mu.Unlock()

	if height > d.height {
		d.logger.Error("GetIDs: height in future", zap.Uint64("requested", height), zap.Uint64("current", d.height))
		return nil, fmt.Errorf("height %d is in the future: %w", height, coreda.ErrHeightFromFuture)
	}

	kvps, ok := d.data[height]
	if !ok {
		d.logger.Debug("GetIDs: no data for height", zap.Uint64("height", height))
		return nil, nil
	}
	...
	d.logger.Debug("GetIDs successful", zap.Int("count", len(ids)))
...
func (d *LocalDA) GetProofs(ctx context.Context, ids []coreda.ID, _ []byte) ([]coreda.Proof, error) {
	d.logger.Debug("GetProofs called", zap.Any("ids", ids))
	blobs, err := d.Get(ctx, ids, nil)
	if err != nil {
		d.logger.Error("GetProofs: failed to get blobs", zap.Error(err))
		return nil, err
	}
	...
	d.logger.Debug("GetProofs successful", zap.Int("count", len(proofs)))
	return proofs, nil
}
...
func (d *LocalDA) Commit(ctx context.Context, blobs []coreda.Blob, _ []byte) ([]coreda.Commitment, error) {
	d.logger.Debug("Commit called", zap.Int("numBlobs", len(blobs)))
	commits := make([]coreda.Commitment, len(blobs))
	for i, blob := range blobs {
		commits[i] = d.getHash(blob)
	}
	d.logger.Debug("Commit successful", zap.Int("count", len(commits)))
	return commits, nil
}
...
func (d *LocalDA) SubmitWithOptions(ctx context.Context, blobs []coreda.Blob, gasPrice float64, _ []byte, _ []byte) ([]coreda.ID, error) {
	d.logger.Info("SubmitWithOptions called", zap.Int("numBlobs", len(blobs)), zap.Float64("gasPrice", gasPrice))
	d.mu.Lock()
	defer d.mu.Unlock()
	...
	d.logger.Info("SubmitWithOptions successful", zap.Uint64("newHeight", d.height), zap.Int("count", len(ids)))
	return ids, nil
}
...
func (d *LocalDA) Submit(ctx context.Context, blobs []coreda.Blob, gasPrice float64, _ []byte) ([]coreda.ID, error) {
	d.logger.Info("Submit called", zap.Int("numBlobs", len(blobs)), zap.Float64("gasPrice", gasPrice))
	d.mu.Lock()
	defer d.mu.Unlock()
	ids := make([]coreda.ID, len(blobs))
	d.height += 1
	...
	d.logger.Info("Submit successful", zap.Uint64("newHeight", d.height), zap.Int("count", len(ids)))
	return ids, nil
}
...
func (d *LocalDA) Validate(ctx context.Context, ids []coreda.ID, proofs []coreda.Proof, _ []byte) ([]bool, error) {
	d.logger.Debug("Validate called", zap.Int("numIDs", len(ids)), zap.Int("numProofs", len(proofs)))
	if len(ids) != len(proofs) {
		d.logger.Error("Validate: id/proof count mismatch", zap.Int("ids", len(ids)), zap.Int("proofs", len(proofs)))
	...
	for i := 0; i < len(ids); i++ {
		...
		d.logger.Debug("Validate result", zap.ByteString("id", ids[i]), zap.Bool("result", results[i]))
	}
	d.logger.Debug("Validate finished", zap.Any("results", results))
	return results, nil
}
