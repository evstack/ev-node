...
func (s *DefaultStore) Rollback(ctx context.Context, height uint64, aggregator bool) error {
	...
	if err != nil && !errors.Is(err, ds.ErrNotFound) {
		...
		if daIncludedHeight > height {
			...
			if aggregator {
				...
				if err := batch.Put(ds.NewKey(getMetaKey(DAIncludedHeightKey)), bz); err != nil {
	...
	for currentHeight > height {
		...
		if err := batch.Delete(ds.NewKey(getHeaderKey(currentHeight))); err != nil {
			return fmt.Errorf("failed to delete header blob in batch: %w", err)
		}

		if err := batch.Delete(ds.NewKey(getDataKey(currentHeight))); err != nil {
			return fmt.Errorf("failed to delete data blob in batch: %w", err)
		}

		if err := batch.Delete(ds.NewKey(getSignatureKey(currentHeight))); err != nil {
			return fmt.Errorf("failed to delete signature of block blob in batch: %w", err)
		}

		hash := header.Hash()
		if err := batch.Delete(ds.NewKey(getIndexKey(hash))); err != nil {
	...
	if err := batch.Put(ds.NewKey(getHeightKey()), heightBytes); err != nil {
	...
	if err := batch.Put(ds.NewKey(getStateAtHeightKey(height)), data); err != nil {
