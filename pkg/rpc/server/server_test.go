...
func TestGetBlock(t *testing.T) {
	...
	t.Run("by height with DA heights", func(t *testing.T) {
		...
		mockStore.On("GetMetadata", mock.Anything, fmt.Sprintf("%s/%d/h", store.EvolveNodeHeightToDAHeightKey, height)).Return(headerDAHeightBytes, nil).Once()
		mockStore.On("GetMetadata", mock.Anything, fmt.Sprintf("%s/%d/d", store.EvolveNodeHeightToDAHeightKey, height)).Return(dataDAHeightBytes, nil).Once()
	...
	t.Run("by height DA heights not found", func(t *testing.T) {
		mockStore.On("GetBlockData", mock.Anything, height).Return(header, data, nil).Once()
		mockStore.On("GetMetadata", mock.Anything, fmt.Sprintf("%s/%d/h", store.EvolveNodeHeightToDAHeightKey, height)).Return(nil, ds.ErrNotFound).Once()
		mockStore.On("GetMetadata", mock.Anything, fmt.Sprintf("%s/%d/d", store.EvolveNodeHeightToDAHeightKey, height)).Return(nil, ds.ErrNotFound).Once()
...
func TestGetBlock_Latest(t *testing.T) {
	...
	mockStore.On("GetMetadata", mock.Anything, fmt.Sprintf("%s/%d/h", store.EvolveNodeHeightToDAHeightKey, latestHeight)).Return(headerDAHeightBytes, nil).Once()
	mockStore.On("GetMetadata", mock.Anything, fmt.Sprintf("%s/%d/d", store.EvolveNodeHeightToDAHeightKey, latestHeight)).Return(dataDAHeightBytes, nil).Once()
