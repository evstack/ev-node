...
func TestDirectTxToDAForceInclusion(t *testing.T) {
	...
	data := types.Data{
		Metadata: &types.Metadata{
			...
			Height:       1,              // This doesn't matter for direct transactions
			Time:         uint64(time.Now().UnixNano()),
			LastDataHash: types.Hash{}, // This doesn't matter for direct transactions
		},
	...
	}, 10*time.Second, 500*time.Millisecond, "Transaction should be included in a block")
