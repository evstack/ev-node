...
// TestSetEvolveNodeHeightToDAHeight tests the SetEvolveNodeHeightToDAHeight method which maps
// ev-node block heights to their corresponding DA heights for both headers and data.
// This method is critical for tracking which DA heights contain specific ev-node blocks.
func TestSetEvolveNodeHeightToDAHeight(t *testing.T) {
	t.Run("Success_WithTransactions", func(t *testing.T) {
		...
		headerKey := fmt.Sprintf("%s/%d/h", storepkg.EvolveNodeHeightToDAHeightKey, height)
		dataKey := fmt.Sprintf("%s/%d/d", storepkg.EvolveNodeHeightToDAHeightKey, height)
		...
		// Call the method
		err := m.SetEvolveNodeHeightToDAHeight(ctx, height)
		require.NoError(err)
