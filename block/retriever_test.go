...
func setupManagerForRetrieverTest(t *testing.T, initialDAHeight uint64) (*Manager, *rollmocks.MockDA, *rollmocks.MockStore, *zap.Logger, *cache.Cache[types.SignedHeader], *cache.Cache[types.Data], context.CancelFunc) {
	t.Helper()
	mockDAClient := rollmocks.NewMockDA(t)
	mockStore := rollmocks.NewMockStore(t)
	mockLogger := zap.NewNop() // Use no-op logger for tests
...
func TestProcessNextDAHeader_HeaderAndDataAlreadySeen(t *testing.T) {
	...
	manager, mockDAClient, _, _, headerCache, dataCache, cancel := setupManagerForRetrieverTest(t, daHeight)
	defer cancel()
...
func TestRetrieveLoop_ProcessError_Other(t *testing.T) {
	...
	// Allow any Debug/Info/Warn
	for _, _ = range []string{"Debug", "Info", "Warn"} {
	...
	// Check if the error was logged (skipped with no-op logger)
	// if !errorLogged.Load() {
