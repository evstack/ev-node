...
func createTestComponents(t *testing.T, config evconfig.Config) (coreexecutor.Executor, coresequencer.Sequencer, *p2p.Client, datastore.Batching, *key.NodeKey, func()) {
	...
	return executor, sequencer, p2pClient, ds, p2pKey, func() {}
}
...
func createNodeWithCleanup(t *testing.T, config evconfig.Config) (*FullNode, func()) {
	executor, sequencer, p2pClient, ds, _, cleanup := createTestComponents(t, config)
	return newTestNode(t, config, executor, sequencer, nil, p2pClient, ds, cleanup)
}
...
func createNodesWithCleanup(t *testing.T, num int, config evconfig.Config) ([]*FullNode, []func()) {
	...
	executor, sequencer, p2pClient, ds, aggP2PKey, stopComponents := createTestComponents(t, config)
	...
	for i := 1; i < num; i++ {
		...
		executor, sequencer, p2pClient, _, nodeP2PKey, stopComponents := createTestComponents(t, config)
		node, err := NewNode(
			config,
			executor,
			sequencer,
