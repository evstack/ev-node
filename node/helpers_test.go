...
func createTestComponents(t *testing.T, config evnodeconfig.Config) (coreexecutor.Executor, coresequencer.Sequencer, coreda.DA, *p2p.Client, datastore.Batching, *key.NodeKey, func()) {
...
func getTestConfig(t *testing.T, n int) evnodeconfig.Config {
	...
	return evnodeconfig.Config{
		RootDir: t.TempDir(),
		Node: evnodeconfig.NodeConfig{
			Aggregator:               true,
			BlockTime:                evnodeconfig.DurationWrapper{Duration: 100 * time.Millisecond},
			MaxPendingHeadersAndData: 1000,
			LazyBlockInterval:        evnodeconfig.DurationWrapper{Duration: 5 * time.Second},
		},
		DA: evnodeconfig.DAConfig{
			BlockTime: evnodeconfig.DurationWrapper{Duration: 200 * time.Millisecond},
			Address:   MockDAAddress,
			Namespace: MockDANamespace,
		},
		P2P: evnodeconfig.P2PConfig{
			ListenAddress: fmt.Sprintf("/ip4/127.0.0.1/tcp/%d", startPort+n),
		},
		RPC: evnodeconfig.RPCConfig{
			Address: fmt.Sprintf("127.0.0.1:%d", 8000+n),
		},
		ChainID:         "test-chain",
		Instrumentation: &evnodeconfig.InstrumentationConfig{},
	}
}
...
func newTestNode(
	...
	node, err := NewNode(
		...
		genesis,
		ds,
		DefaultMetricsProvider(evnodeconfig.DefaultInstrumentationConfig()),
		logging.Logger("test"),
		NodeOptions{},
	)
	require.NoError(t, err)
...
func createNodeWithCleanup(t *testing.T, config evnodeconfig.Config) (*FullNode, func()) {
...
func createNodesWithCleanup(t *testing.T, num int, config evnodeconfig.Config) ([]*FullNode, []func()) {
	t.Helper()
