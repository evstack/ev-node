...
func TestBatchQueueThrottlingWithDAFailure(t *testing.T) {
	...
	config.Node.BlockTime = evnodeconfig.DurationWrapper{Duration: 100 * time.Millisecond}
	config.DA.BlockTime = evnodeconfig.DurationWrapper{Duration: 1 * time.Second} // Longer DA time to ensure blocks are produced first
