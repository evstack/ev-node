...
func TestTxGossipingMultipleNodesNoDA(t *testing.T) {
	...
	config.DA.BlockTime = evnodeconfig.DurationWrapper{Duration: 100 * time.Second}
	numNodes := 3
...
func TestFastDASync(t *testing.T) {
	...
	config.Node.BlockTime = evnodeconfig.DurationWrapper{Duration: 2 * time.Second}
	config.DA.BlockTime = evnodeconfig.DurationWrapper{Duration: 1 * time.Second}
...
func TestSingleSequencerTwoFullNodesBlockSyncSpeed(t *testing.T) {
	...
	config := getTestConfig(t, 1)
	config.Node.BlockTime = evnodeconfig.DurationWrapper{Duration: 100 * time.Millisecond} // fast block time
	config.DA.BlockTime = evnodeconfig.DurationWrapper{Duration: 10 * time.Second}         // slow DA block time
